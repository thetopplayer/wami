...
if (mysqli_num_rows($result_set) > 0) {
    ...
    while ($row = mysqli_fetch_array($result_set)) {
        ...
        try {
            ...
            if (!$result) {
                $response["ret_code"] = 1;
                $response["message"] = "update_for_delete_account.php: Problem updating image gallery data. MySQL Error: " .mysqli_error($con);
                $con->rollback();
                $con->autocommit(TRUE);
                echo json_encode($response);
                exit(-1);
            ...
            if (!$result) {
                $response["ret_code"] = 1;
                $response["message"] = "update_for_delete_account.php: Problem updating audio file data. MySQL Error: " .mysqli_error($con);
                $con->rollback();
                $con->autocommit(TRUE);
                echo json_encode($response);
                exit(-1);
            ...
            if (!$result) {
                $response["ret_code"] = 1;
                $response["message"] = "update_for_delete_account.php: Problem updating text file data. MySQL Error: " .mysqli_error($con);
                $con->rollback();
                $con->autocommit(TRUE);
                echo json_encode($response);
                exit(-1);
            ...
            if (!$result) {
                $response["ret_code"] = 1;
                $response["message"] = "update_for_delete_account.php: Problem updating pdf file data. MySQL Error: " .mysqli_error($con);
                $con->rollback();
                $con->autocommit(TRUE);
                echo json_encode($response);
                exit(-1);
            ...
            $sql =  "UPDATE user SET delete_ind = 1, modified_date = NOW() WHERE user_id = " .$user_id;
            $result = mysqli_query($con, $sql) or die(mysqli_error($con));
            if (!$result) {
                $response["ret_code"] = 1;
